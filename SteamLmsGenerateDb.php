...
echo "done"; 
/* // Initiate variables
$dbHost = $_ENV['VS_DB_HOST'];
$dbName = $_ENV['VS_DB_NAME'];
$dbUser = $_ENV['VS_DB_USER'];
$dbPassword = $_ENV['VS_DB_PWD'];
$adminPseudo = $_ENV['ADMIN_PSEUDO'];
$adminPassword = $_ENV['ADMIN_PASSWORD'];
$adminEmail = $_ENV['ADMIN_EMAIL'];
$db=null;

// Create PDO connection without $dbName else it will not work
try {
...
// reset PDO and instanciate the main PDO connextion
$db=null;

// Create PDO connection to add the tables
try {
...
// Load the sql file and create tables
$queryFile = file_get_contents("steam-lms-generate-db.sql");
$req = $db->prepare($queryFile);
$res = $req->execute();
...
INSERT INTO users(firstname, pseudo, surname, password) VALUES (?,?,?,?)
");
$req->execute(array($adminPseudo, $adminPseudo, $adminPseudo,$passwordHash));
...
VALUES (?, ?, ?, ?, ?, ?)
");
$req->execute(array($lastInsertedId,$adminEmail,0,1,1,0));
